...
Puis, sur GitHub : onglet **Releases** → `v0.4.18` → télécharge **`Plume-Setup-0.4.18.exe`**.
...
Sorties : `dist\Plume\Plume.exe` (portable) et `dist\installer\Plume-Setup-0.4.18.exe`.
...
1. Lance `Plume-Setup-0.4.18.exe` (installation sans droits admin, dans ton profil utilisateur).
