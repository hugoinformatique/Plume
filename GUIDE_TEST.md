...
Puis, sur GitHub : onglet **Releases** → `v0.4.16` → télécharge **`Plume-Setup-0.4.16.exe`**.
...
Sorties : `dist\Plume\Plume.exe` (portable) et `dist\installer\Plume-Setup-0.4.16.exe`.
...
1. Lance `Plume-Setup-0.4.16.exe` (installation sans droits admin, dans ton profil utilisateur).
